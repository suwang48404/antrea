...
func configureContainerAddr(netns ns.NetNS, containerInterface *current.Interface, result *current.Result) error {
	...
	if containerVeth != nil {
		...
		go func() {
			_ = netns.Do(func(containerNs ns.NetNS) error {
				count := 0
				for count < 3 {
					select {
...
func (pc *podConfigurator) configureInterfaces(
	...
	if err := pc.connectInterfaceToOVS(podName, podNameSpace, containerID, hostIface, containerIface, result.IPs); err != nil {
		return fmt.Errorf("failed to connect to ovs for contaiiner %s: %v", containerID, err)
	}
	defer func() {
		if !success {
...
// connectInterfaces connect an existing interface to ovs br-int.
func (pc *podConfigurator) connectInterfaceToOVS(
	...
	defer func() {
		if err != nil {
			_ = pc.ofClient.UninstallPodFlows(ovsPortName)
		}
	}()
...
func (pc *podConfigurator) connectInterceptedInterface(
	...
	if len(br) > 0 {
		if err := util.DetachDevFromBridge(intf, br); err != nil {
			return fmt.Errorf("connectInterceptedInterface failed to detach: %w", err)
		}
	}
	...
	return pc.connectInterfaceToOVS(podName, podNameSpace, containerID, hostIface,
		containerIface, containerIPs)
}

// disconnectInterceptedInterface disconnects intercepted interface from ovs br-int.
func (pc *podConfigurator) disconnectInterceptedInterface(podName, podNamespace, containerID string, isL3 bool) error {
	containerConfig, found := pc.ifaceStore.GetContainerInterface(podName, podNamespace)
	if !found {
		klog.V(2).Infof("Did not find the port for container %s in local cache", containerID)
		return nil
