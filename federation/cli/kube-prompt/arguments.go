...
func (c *Completer) argumentsCompleter(namespace string, args []string) []prompt.Suggest {
	...
	case "get":
		...
		if len(args) == 3 {
			...
			// Antrea
			case "antreaneworkpolicies":
				return prompt.FilterContains(getANPSuggestions(c.antreaClient, namespace), third, true)
			case "federations":
				return prompt.FilterContains(getFedSuggestions(c.antreaClient, namespace), third, true)
			case "clusterclaims":
				return prompt.FilterContains(getCCSuggestions(c.antreaClient, namespace), third, true)
			case "clusterroles":
				return prompt.FilterContains(getClusterRoleSuggestions(c.antreaClient, namespace), third, true)
			case "clusterrolebindings":
				return prompt.FilterContains(getClusterRoleBindingSuggestions(c.antreaClient, namespace), third, true)
			}
		}
	case "describe":
		second := args[1]
