...
// AntreaPlus adds cluster context to go-client.
func NewCompleter(clusterContext string, antreaClient client2.Client) (*Completer, error) {
	...
	case "exec", "logs", "run", "attach", "port-forward", "cp":
		if option == "-c" || option == "--container" {
			...
			if cmdArgs == nil || len(cmdArgs) < 2 {
				suggestions = getContainerNamesFromCachedPods(c.client, namespace)
			} else {
				suggestions = getContainerName(c.client, namespace, cmdArgs[1])
			}
			return prompt.FilterHasPrefix(
				suggestions,
				d.GetWordBeforeCursor(),
